...
import '../skill_tree_visualization.dart';
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          SizedBox(height: 8),
          Text(
            'Estimated time: ${node.estimatedTimeHours} hours',
            style: TextStyle(
              fontSize: 16,
          ...
          if (node.resources.isNotEmpty) ...[
            ...
            SizedBox(height: 8),
            ...node.resources.map((resource) => Padding(
          ...
          if (node.subskills.isNotEmpty) ...[
            ...
            SizedBox(height: 8),
            ...node.subskills.map((subskill) => Padding(
              padding: EdgeInsets.only(bottom: 8),
              child: Text(
                '• ${subskill.name} (${subskill.estimatedTimeHours} hrs)',
...
enum SkillTreeViewMode { tree, list }

class _SkillTreeScreenState extends State<SkillTreeScreen> {
  SkillNode? _selectedNode;
  SkillTreeViewMode _viewMode = SkillTreeViewMode.list;
  ...
  void _switchView(SkillTreeViewMode mode) {
    setState(() {
      _viewMode = mode;
    });
  }
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            tooltip: 'Tree View',
            onPressed: () => _switchView(SkillTreeViewMode.tree),
            color: _viewMode == SkillTreeViewMode.tree ? Colors.amber : null,
          ),
          IconButton(
            icon: Icon(Icons.view_list),
            tooltip: 'List View',
            onPressed: () => _switchView(SkillTreeViewMode.list),
            color: _viewMode == SkillTreeViewMode.list ? Colors.amber : null,
          ),
        ],
      ),
      body: Stack(
        children: [
          if (_viewMode == SkillTreeViewMode.tree)
            Column(
              children: [
                Expanded(
                  flex: 2,
                ...
                Expanded(
                  ...
                  child: SingleChildScrollView(
                    child: Padding(
                      ...
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: widget.skillTree.skills.map((node) => _buildSkillNode(node)).toList(),
                      ),
                    ),
                  ),
                ),
          ...
          if (_viewMode == SkillTreeViewMode.list)
            SkillTreeListView(skillTree: widget.skillTree),
  ...
  Widget _buildSkillNode(SkillNode node, {int indent = 0}) {
    return Padding(
      ...
      child: Column(
        ...
        children: [
          Text(
            '${'  ' * indent}• ${node.name} (${node.estimatedTimeHours} hrs)',
            style: TextStyle(fontWeight: FontWeight.bold),
          ),
          ...node.resources.map((r) => Padding(
                padding: EdgeInsets.only(left: 16.0),
                child: Text('- ${r.title}: ${r.url}', style: TextStyle(fontSize: 12)),
              )),
          ...node.subskills.map((sub) => _buildSkillNode(sub, indent: indent + 1)),
        ],
      ),
    );
  }
